...
class FDC_deep_preact(nn.Module):
    def __init__(self, z_hidden, sub_in_dim, block=PreActBlock, num_blocks=[2,2,2,2], num_classes=10):
        ...
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, special=128+16)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.bn = nn.BatchNorm2d(512 * block.expansion)
        self.linear = nn.Linear(512*block.expansion, num_classes)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, special=0):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for c, stride in enumerate(strides):
            if c ==0 and special>0:
                layers.append(block(special, planes, stride))
            else:
                layers.append(block(self.in_planes, planes, stride))
            self.in_planes = planes * block.expansion
        return nn.Sequential(*layers)
    ...
    def forward(self, z1, z2, test=True):
        ...
        if test:
            # z1, [B, C, H, W]   -> [B, 1, C, H, W]
            z1 = z1.unsqueeze(1)
            z1 = z1.repeat(1, bs, 1, 1, 1)
            z2 = z2.unsqueeze(0)  # -> [1, B, C', H, W]
            z2 = z2.repeat(bs, 1, 1, 1, 1)
...
class FDC_deep_preact_earlymerge(nn.Module):
    def __init__(self, z_hidden, sub_in_dim, block=PreActBlock, num_blocks=[2,2,2,2], num_classes=10):
        super(FDC_deep_preact_earlymerge, self).__init__()

        self.in_planes = 64

        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, special=64+16)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        self.bn = nn.BatchNorm2d(512 * block.expansion)
        self.linear = nn.Linear(512*block.expansion, num_classes)

        self.conv1 = nn.Conv2d(3, 16, kernel_size=5, stride=1, padding=2, bias=False) # hurt final result
    ...
    def forward(self, z1, z2, test=True):
        ...
        if test:
            ...
            hh = hh.view(bs * bs, hh.size(2), hh.size(3), hh.size(4))

            out = self.layer2(hh)
            out = out.view(bs, bs, out.size(1), out.size(2), out.size(3))
