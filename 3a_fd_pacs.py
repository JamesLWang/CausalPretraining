...
def main():
    ...
    classifier = FDC(hidden_dim=latent_dim, cat_num=7, drop_xp=args.drop_xp).cuda()
    classifier = nn.DataParallel(classifier)
    classifier.train()

    if args.linear:
        aux_classifier = FCClassifier(hidden_dim=latent_dim, out_dim=7)
    else:
        aux_classifier = FC2Classifier(hidden_dim=latent_dim, out_dim=7)
